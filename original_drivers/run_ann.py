...
from drivers.run_warmup import dev_evaluation
from drivers.run_ann import compute_total_grad_L2_norm
...
def intrain_dev_eval(args, global_step, model, tb_writer):
    model.eval()
    for i_dev, data_name in enumerate(args.intraindev_data_name.split(',')):
        data_path = args.intraindev_data_dir.split(',')[i_dev]
        dev_result_dict = dev_evaluation(args, data_path, model)
        if is_first_worker():
...
def intrain_save_checkpoint(args, global_step,
    ...
    if is_first_worker():
        # identical with the one from drivers/run_ann
...
def train(args, model, tokenizer, query_cache, passage_cache):
    ...
    tr_grad_norm = {k: 0.0 for k in model_parts}
    tr_grad_norm.update({k+'-clipped': 0.0 for k in model_parts})
    ...
    while global_step < args.max_steps:
        ...
        if step % args.gradient_accumulation_steps == 0:
            update_frequency = args.logging_steps if args.fix_refresh_rate==0 else args.fix_refresh_rate
            if global_step % update_frequency == 0:
                if args.fix_refresh_rate > 0:
                    # check until new ann training data is available
                    ann_no = last_ann_no
                ...
                if ann_path is not None and ann_no != last_ann_no:
                    ...
                    # if global_step > 0 and actual_refresh_rate is None:
                    #     # the first time refreshing
                    #     actual_refresh_rate = global_step
                    # half_eval_done = False
                    # eval_and_save = global_step > 0
                    # prev_refresh_gstep = global_step
                    ...
                    if is_first_worker():
                        ...
                        epoch_num += 1
                        tb_writer.add_scalar('epoch', epoch_num, global_step)
                    last_ann_no = ann_no
                    
                # if actual_refresh_rate is not None and \
                #         (global_step - prev_refresh_gstep) * 2 > actual_refresh_rate:
                #     # past a half
                #     if not half_eval_done:
                #         eval_and_save = True
                #         half_eval_done = True

                    if global_step > 0:
                        intrain_dev_eval(
                            args, global_step, model, tb_writer)
        ...
        if step % args.gradient_accumulation_steps == 0:
            ...
            for model_part, params in model_parts_params.items():
                tr_grad_norm[model_part] += compute_total_grad_L2_norm(params)
            ...
            for model_part, params in model_parts_params.items():
                tr_grad_norm[model_part+'-clipped'] += compute_total_grad_L2_norm(params)
            ...
            if args.logging_steps > 0 and global_step % args.logging_steps == 0:
                ...
                for model_part in model_parts:
                    logs[f'grad_norm-{model_part}'] = tr_grad_norm[model_part] / args.logging_steps
                    logs[f'grad_norm-{model_part}-clipped'] = tr_grad_norm[model_part+'-clipped'] / args.logging_steps
                    tr_grad_norm[model_part] = 0.0
                    tr_grad_norm[model_part+'-clipped'] = 0.0
                learning_rate_scalar = scheduler.get_lr()[0]
                logs["learning_rate"] = learning_rate_scalar
                logs["loss_ranking"] = loss_scalar
                tr_loss = 0
